...
/*   Updated: 2025/04/24 17:49:00 by juanrome         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_sqrt(int nmb) {
    ...
    i = 2;
    while (i * i < nmb) i++;
    if (i * i - nmb < nmb - (i - 1) * (i - 1)) return (i);
    return (i - 1);
}
...
int count_index(t_node *stack, int index) {
    int pos = 0;
    while (stack) {
        if (stack->index == index) return pos;
        stack = stack->next;
        pos++;
    }
    return -1;
...
void k_sort2(t_stacks *stacks, int len, int *count) {
    while (--len >= 0) {
        if (count_index(stacks->stack_b, len) <= stack_size(stacks->stack_b) / 2) {
